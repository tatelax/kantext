...
* Make Inbox/todo, in progress, and done impossible to remove. If they dont exist, the web server should add them
* Add a pencil icon to rename custom columns but the default ones should not be able to be deleted
X Make task view wider
X Lower opacity on blockers behind task view and dialog boxes
* Add red exclamation mark to the left of task titles (or left of the icon indicating the task has ) stale tasks. Define what stale means in config.json
